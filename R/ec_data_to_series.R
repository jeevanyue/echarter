...
ec_data_to_series <- function(ec, data, mapping, ..., dim = FALSE) {
  ...
  if(type %in% c("radar", "parallel")){
    ...
    data_ <- data %>%
      group_by_("group") %>%
      do(data = ec_list_parse(select_(., quote(-group)))) %>%
      ungroup()
  }else if(type %in% c('tree', 'treemap', 'sunburst')){
    data_ <- data_tree(data, type = type)
    data_ <- data_frame(data = list(data_))
      ...
      if(outline){
        ...
        if(boxplot_layout == "horizontal"){
          dat_outline <- map2_df(seq(nrow(dat_box)), dat_box$data, function(x, y){
            if (length(y$out) > 0)
              d <- data_frame(name = dat_box$name[x], data = list(ec_dim(data_frame(x = y$group - 1, y = y$out))))
            else
              d <- data_frame()
            d
          })
          ...
          dat_outline <- map2_df(seq(nrow(dat_box)), dat_box$data, function(x, y){
            if (length(y$out) > 0)
              d <- data_frame(name = dat_box$name[x], data = list(ec_dim(data_frame(x = y$out, y = y$group - 1))))
            else
              d <- data_frame()
            d
          })
    ...
    data_ <- data %>%
      group_by_("group") %>%
      do(data = list_parse_data(select_(., quote(-group)), coordinateSystem = coordinateSystem, dim = dim)) %>%
      ungroup()
